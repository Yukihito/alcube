#include "TextureInstanceColorShader.h"

namespace alcube::drawing::shaders::directionallight {
  TextureInstanceColorShader::TextureInstanceColorShader(
    alcube::utils::FileUtil *fileUtil,
    alcube::drawing::Context &context
  ) {
    std::string vertexShaderCodeStr = fileUtil->readFile("../src/shaders/directional-light/texture-instance-color.vertexshader");
    std::string fragmentShaderCodeStr = fileUtil->readFile("../src/shaders/directional-light/texture-instance-color.fragmentshader");
    ...
    instanceAttributes.push_back(new InstanceAttribute(COLORS, 8));
  }
}
